...
| CB-10d2 | Drop `transform_blocks` + `transform_block_runs` | S–M | ✅ | CB-10d1 ✅ | — |
...
**Closed.** All 20 of 20 units are done and all four phase gates are signed off. The
Phase 4 Gate was signed 2026-09-11. CB-10d2, the one irreversible unit, landed
2026-09-10 (`d208aee1`). Its migration `0049` drops the tables on each environment
when that environment deploys, so for production the drop happens at the
`main` promotion, not at the commit.
...
## Phase 4 Gate ✅ PASSED 2026-09-11

Signed by the reviewer on `f57bf806`. Restated against CB-10a..d2, since the
original single CB-10 no longer exists.

- [x] **CB-10d2** ✅ — 2026-09-10, `d208aee1`. Four statements: two `DROP TABLE ...
      CASCADE` and two `DROP TYPE`. They were generated by `db:generate` and read
      line by line. Reviewer-verified at landing, and recorded in full in the
      commit message:
      - A complete chain replay into a fresh database, then a catalog probe:
        the transform tables are gone, and `transform_block_language`
        **survives**, because it backs the live `lifecycle_hooks.language` and
        `document_hooks.language` columns.
      - Gates re-run: counts exactly at baseline (338/3888, 148/1383 + 3). Only
        five assertions were removed, all querying the now-absent table.
      - Production counts taken immediately before: 1 `transform_blocks` row,
        0 runs. The owner ruled that row is dropped with the table, with no
        export.
...
- [x] `npm run type-check` → 0 errors · `npm run lint` → clean ·
      `check:strict-zones` 6/6. Re-run on `f57bf806`, 2026-09-11.
- [x] `test:fast` + `test:integration` → green after CB-10a..d2:
      - `test:fast`: **338 files / 3890**. That is CB-10d2's 3888 plus the two
        guard tests added by RLS-B1 and the AI-probe fix.
      - `test:integration` (owner role): 148 files, 1382 passed + 3 skipped.
        One failure, `portability.export`'s "leaves no temp file behind", is a
        parallel-worker race on the shared temp dir: the count went 1 → 0
        mid-test, and the file passed 19/19 on each of two isolated runs.
      - The same suite as a **non-owner role** (the RLS gate): **148/148 files,
        1386/1386**.
      - The reductions from the Phase 3 baseline (339/3867, 149/1385) are each
        named in the per-ticket notes above: CB-10a −7, CB-10c −8/+1, CB-10d1
        −4/+6/−1, and CB-10d2 exactly at baseline.
- [x] `server/services/scripting/` untouched across the whole initiative:
      ...
      on `f57bf806`, re-checked at sign-off as required.
- [x] **CI green on `dev` for the first time since before Phase 4.** All three
      workflows passed on `f57bf806`: RLS Enforcement Gate, Deployment Safety
      Check and TypeScript Strict Mode Check.
...
### The two red CI gates — both resolved 2026-09-11

This gate once named two red gates "outside this initiative's control". One of
those diagnoses turned out to be wrong:

- **RLS Enforcement Gate** (`RLS-B1`). The filing called it a probable CB-9a-1
  fixture problem. It was production code:
  - `EnvelopeBuilder` re-read RLS-covered tables on the bare pool after its
    tenant transaction closed.
  - The document-delivery enqueue and worker had no tenant scoping at all.

  Fixed in `760353b6`.
- **Deployment Safety Check** (`DEP-B1`). The audit half was fixed by
  `265cbeb0`. The check then **stayed red for a different reason**: its SSRF grep
  caught a raw `fetch(` in the AI readiness probe added by `cf673f30`. Fixed in
  `f57bf806` by routing the probe through `safeFetch`, and proven live against
  the vendor.
