      ...
      <div className={`${brand.maxw}`}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {CONTENT.cards.map(({ icon: Icon, title, tag, body }, i) => (
            ...
            >
              <div
                className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-indigo-50 text-indigo-700 mb-4"
                aria-hidden="true"
              >
                <Icon className="h-6 w-6" />
              </div>
              <div className="font-bold text-lg mb-1">{title}</div>
              <div className="inline-block px-2 py-0.5 rounded bg-indigo-50 text-xs font-semibold text-indigo-700 mb-2">
                {tag}
              </div>
