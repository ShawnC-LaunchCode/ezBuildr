  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <ListColumns list={list} variableName={variableName} onCopy={handleCopyReference} />
        <ListQuickRefs variableName={variableName} onCopy={handleCopyReference} />
...
function ListMetadata({ list }: { list: ListVariable }) {
  return (
    <>
...
function ListColumns({ list, variableName, onCopy }: { list: ListVariable; variableName: string; onCopy: (ref: string) => void }) {
  return (
        ...
        {list.columns.map((col) => (
          ...
          >
            ...
            <Button
              ...
              onClick={() => { onCopy(`${variableName}.rows[i].${col.id}`); }}
...
function ListQuickRefs({ variableName, onCopy }: { variableName: string; onCopy: (ref: string) => void }) {
  return (
    <div className="space-y-2 pt-2 border-t">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between p-2 bg-muted/30 rounded text-xs font-mono group hover:bg-muted/50">
          ...
          <Button
            ...
            onClick={() => { onCopy(`${variableName}.count`); }}
          >
            <Copy className="w-3 h-3" />
          </Button>
        </div>
        <div className="flex items-center justify-between p-2 bg-muted/30 rounded text-xs font-mono group hover:bg-muted/50">
          ...
          <Button
            ...
            size="icon"
            className="h-5 w-5 opacity-0 group-hover:opacity-100"
            onClick={() => { onCopy(`${variableName}.rows[0]`); }}
          >
            <Copy className="w-3 h-3" />
          </Button>
        </div>
...
function ListPreview({ list }: { list: ListVariable }) {
