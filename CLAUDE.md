...
7. **Tenant Isolation:** service-layer `tenant_id` scoping, the `withTenant` / `withCurrentTenant` helpers, and Postgres RLS — **enforced in all three environments since 2026-09-13** (38 tables, FORCE on; the app connects as the non-owner `ezbuildr_app`, admin cross-tenant reads go through `ADMIN_DATABASE_URL`). **A query on a tenant table that runs outside a tenant transaction now returns ZERO rows in production** — not an error, so it looks like missing data. Every new query on a tenant table must run inside one, and new tenant tables need an RLS policy; the required `RLS Enforcement Gate` catches most misses. Never set the tenant GUC session-level — see `docs/architecture/TENANT_ISOLATION_RLS.md` (SEC-051) and `docs/deployment/RLS4_CUTOVER.md`
