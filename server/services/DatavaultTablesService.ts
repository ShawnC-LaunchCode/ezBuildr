...
export class DatavaultTablesService {
  ...
  async createTable(data: InsertDatavaultTable, tx?: DbTransaction): Promise<DatavaultTable> {
    return this.withTx(data.tenantId, tx, async (scopedTx) => {
      ...
      if (data.databaseId) {
        ...
        const database = await datavaultDatabasesRepository.findById(data.databaseId, scopedTx);
        if (database) {
        ...
        const canManage = await canManageOrg(data.ownerUserId, ownerUuid, scopedTx);
        if (!canManage) {
  ...
  async getTableWithColumns(tableId: string, tenantId: string, tx?: DbTransaction) {
    return this.withTx(tenantId, tx, async (scopedTx) => {
      const table = await this.verifyTenantOwnership(tableId, tenantId, scopedTx);
  ...
  async getTableSchema(tableId: string, tenantId: string, tx?: DbTransaction) {
    return this.withTx(tenantId, tx, async (scopedTx) => {
      await this.verifyTenantOwnership(tableId, tenantId, scopedTx);
      const schema = await this.tablesRepo.getSchema(tableId, scopedTx);
  ...
  async listTables(tenantId: string, userId: string, tx?: DbTransaction): Promise<DatavaultTable[]> {
    return this.withTx(tenantId, tx, (scopedTx) =>
      this.tablesRepo.findByTenantAndUser(tenantId, userId, scopedTx));
  }
  ...
  async listTablesWithStats(tenantId: string, userId: string, tx?: DbTransaction) {
    return this.withTx(tenantId, tx, async (scopedTx) => {
      const tables = await this.tablesRepo.findByTenantAndUser(tenantId, userId, scopedTx);

      if (tables.length === 0) {return [];}
      const tableIds = tables.map(t => t.id);
      const [columnCounts, rowCounts] = await Promise.all([
        this.columnsRepo.countByTableIds(tableIds, scopedTx),
        this.rowsRepo.countByTableIds(tableIds, false, scopedTx),
      ]);
  ...
  async updateTable(
    ...
    return this.withTx(tenantId, tx, async (scopedTx) => {
      ...
      if (updateData.name && !updateData.slug) {
        ...
        updateData.slug = await this.ensureUniqueSlug(tenantId, baseSlug, tableId, scopedTx);
      }

      // If slug provided, ensure it's unique
      if (updateData.slug) {
        updateData.slug = await this.ensureUniqueSlug(tenantId, updateData.slug, tableId, scopedTx);
      }

      return this.tablesRepo.update(tableId, updateData, scopedTx);
    });
  }
  ...
  async deleteTable(tableId: string, tenantId: string, tx?: DbTransaction): Promise<void> {
    await this.withTx(tenantId, tx, async (scopedTx) => {
      await this.verifyTenantOwnership(tableId, tenantId, scopedTx);
      await this.tablesRepo.delete(tableId, scopedTx);
    });
  }
  ...
  async getTableBySlug(
    ...
    return this.withTx(tenantId, tx, (scopedTx) =>
      this.tablesRepo.findByTenantAndSlug(tenantId, slug, scopedTx));
  }
  ...
  async isSlugAvailable(
    ...
    return this.withTx(tenantId, tx, async (scopedTx) =>
      !(await this.tablesRepo.slugExists(tenantId, slug, excludeId, scopedTx)));
  }
  ...
  async moveTable(
    ...
    return this.withTx(tenantId, tx, async (scopedTx) => {
      ...
      if (databaseId) {
        ...
        if (userId) {
          const hasDatabaseAccess = await datavaultAclService.hasDatabaseRole(userId, databaseId, 'edit', scopedTx);
      ...
      return this.tablesRepo.update(tableId, { databaseId, ...ownerUpdate }, scopedTx);
    });
  }

  async getTableAccess(tableId: string, tenantId: string, userId: string, tx?: DbTransaction): Promise<{
    entries: DatavaultTableAccess[];
    currentUserRole: AccessRole;
  }> {
    return this.withTx(tenantId, tx, async (scopedTx) => {
      await this.requirePermission(userId, tableId, tenantId, 'read', scopedTx);
      const [entries, currentUserRole] = await Promise.all([
        this.accessRepo.findByTableId(tableId, scopedTx),
        datavaultAclService.resolveRoleForTable(userId, tableId, scopedTx),
      ]);
  ...
  async grantTableAccess(
    ...
    return this.withTx(tenantId, tx, async (scopedTx) => {
      await this.requirePermission(requestorId, tableId, tenantId, 'owner', scopedTx);
      const results: DatavaultTableAccess[] = [];
      for (const entry of entries) {
        const acl = await this.accessRepo.upsert(tableId, entry.principalType, entry.principalId, entry.role, scopedTx);
        results.push(acl);
      }
      return results;
    });
  ...
  async revokeTableAccess(
    ...
    await this.withTx(tenantId, tx, async (scopedTx) => {
      await this.requirePermission(requestorId, tableId, tenantId, 'owner', scopedTx);
      await this.accessRepo.deleteManyByPrincipals(tableId, entries, scopedTx);
    });
  }
  ...
  async transferOwnership(
    ...
    // RLS-2b: target bundled into one object (was two positional params) to
    // stay under max-params once `tx` was added — see TransferService for
    // the same shape.
    ...
    return this.withTx(tenantId, tx, async (scopedTx) => {
      ...
      const table = await this.verifyTenantOwnership(tableId, tenantId, scopedTx);
      await this.requirePermission(userId, tableId, tenantId, 'owner', scopedTx);
      if (targetOwnerType === 'org' && !(await canManageOrg(userId, targetOwnerUuid, scopedTx))) {
