...
export class LogicService {
   ...
   *
   * RVP-2: every entry point below used to re-read the LIVE `pages`/
  ...
  async buildContext(
    ...
    for (const rule of logicRules) {
      if (rule.action === "hide") {
        if (rule.targetType === "page" && rule.targetPageId) {
          if (!pageHideRulesMap.has(rule.targetPageId)) {
            pageHideRulesMap.set(rule.targetPageId, []);
          }
          pageHideRulesMap.get(rule.targetPageId)!.push(rule);
  ...
  /**
   * Evaluate logic and determine next page for a workflow run
   *
   * RVP-2: sourced from the run's pinned definition (`RunDefinitionProvider`)
   * rather than the live `pages`/`steps`/`logic_rules` tables, so editing
   ...
   * @param currentPageId - Current page ID (null if starting)
   * @returns Navigation result with next page and visibility info
   */
  async evaluateNavigation(
    ...
    const { visiblePages, visibleSteps, requiredSteps: visibleRequiredSteps } = visibility;

    // Calculate normal next page
    const nextPageId = calculateNextPage(
      currentPageId,
      pages.map((s) => ({ id: s.id, order: s.order })),
      visiblePages
    );

    // Resolve final next page (considering skip logic)
    ...
    return {
      ...
      skipToPageId: visibility.ruleEvaluation.skipToPageId,
      nextPageId: resolvedNextPageId,
      currentProgress,
    };
  }
  ...
  private calculateProgress(
    ...
    // Get sorted visible pages
    const sortedVisiblePages = pages
      .filter((s) => visiblePages.has(s.id))
    ...
    const currentIndex = sortedVisiblePages.findIndex((s) => s.id === currentPageId);
    ...
    const progress = Math.round(((currentIndex + 1) / sortedVisiblePages.length) * 100);

    return Math.min(100, Math.max(0, progress));
  }

  /**
   * Check if a page is visible based on logic rules and current data
   *
   * @param workflowId - Workflow ID
   * @param pageId - Page ID to check
   * @param data - Current step values (key = stepId or alias, value = step value)
   * @returns true if page is visible, false otherwise
   */
  async isPageVisible(
    ctx: LogicContext,
    pageId: string
  ): Promise<boolean> {
    return this.evaluateContextVisibility(ctx).visiblePages.has(pageId);
  }
