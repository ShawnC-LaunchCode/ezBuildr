...
import { calculateNextSection, evaluateWorkflowVisibility, resolveNextSection } from "@shared/workflowLogic";
...
/**
 * A linear three-section workflow with no rules — the default sequential
 * path. Mirrors `tests/fixtures/workflowMap.ts#linearThreeSections`.
 */
function linearThreeSections(): SimulateWorkflowPathInput {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1 },
    { id: "section-c", title: "Section C", order: 2 },
  ];
  return { sections, steps: [], rules: [], data: {}, resolveAlias: noResolveAlias };
}

/**
 * A forward `skip_to` rule, conditioned on a real step value: when
 * `step-a-trigger` is true, section A routes straight to section C,
 * skipping section B. Mirrors
 ...
 */
function forwardSkipWorkflow(): { sections: WorkflowMapSectionInput[]; steps: WorkflowMapStepInput[]; rules: MapRuleFixture[] } {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1 },
    { id: "section-c", title: "Section C", order: 2 },
  ...
  return { sections, steps, rules };
}

/**
 * A backward `skip_to` rule targeting the very first section — a no-op at
 ...
 */
function backwardSkipWorkflow(): { sections: WorkflowMapSectionInput[]; steps: WorkflowMapStepInput[]; rules: MapRuleFixture[] } {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1 },
    { id: "section-c", title: "Section C", order: 2 },
...
/**
 * Two `skip_to` rules target the *same* section (`section-d`) from two
 * *different* origin sections. `rule-from-A` (order 5) is placed **first**
 ...
 * `action`/`targetType`/`targetSectionId` (ignoring both `order` and whether
 ...
 */
function ambiguousSkipTargetWorkflow(): { sections: WorkflowMapSectionInput[]; steps: WorkflowMapStepInput[]; rules: MapRuleFixture[] } {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1 },
    { id: "section-d", title: "Section D", order: 2 },
  ];
  const steps: WorkflowMapStepInput[] = [
    { id: "step-a-trigger", sectionId: "section-a", type: "short_text", title: "From A?" },
    { id: "step-b-trigger", sectionId: "section-b", type: "short_text", title: "From B?" },
  ];
  const rules: MapRuleFixture[] = [
    {
      id: "rule-from-A",
...
/**
 * Section B is hidden unless `has_pet` is true. Mirrors
 * `tests/fixtures/workflowMap.ts#workflowWithConditionalSection`'s
 * `visibleIf`, applied to a section that sits *between* two always-visible
 * ones so the omitted middle section is unambiguous.
 */
function hiddenMiddleSectionWorkflow(): SimulateWorkflowPathInput {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1, visibleIf: isTrue("has_pet") },
    { id: "section-c", title: "Section C", order: 2 },
  ];
  return { sections, steps: [], rules: [], data: {}, resolveAlias: noResolveAlias };
}
...
/**
 * Malformed input: `section-a` appears twice, at orders 0 and 2, under the
 ...
 * `calculateNextSection` level: `findIndex` always resolves `'section-a'` to
...
function oscillatingMalformedWorkflow(): SimulateWorkflowPathInput {
  const sections: WorkflowMapSectionInput[] = [
    { id: "section-a", title: "Section A", order: 0 },
    { id: "section-b", title: "Section B", order: 1 },
    { id: "section-a", title: "Section A (duplicate)", order: 2 },
  ];
  return { sections, steps: [], rules: [], data: {}, resolveAlias: noResolveAlias };
}

/**
 * Drives `evaluateWorkflowVisibility`, `calculateNextSection` and
 * `resolveNextSection` directly, in the same order and with the same
...
function serverStyleWalk(input: SimulateWorkflowPathInput): string[] {
  ...
  const orderedSections = sections.map((s) => ({ id: s.id, order: s.order }));
  const visited: string[] = [];
  let current: string | null = null;

  for (let i = 0; i < sections.length + 1; i++) {
    const visibility = evaluateWorkflowVisibility({ sections, steps, rules, data, resolveAlias });
    const nextSectionId = calculateNextSection(current, orderedSections, visibility.visibleSections);
    const resolved = resolveNextSection(
      current,
      nextSectionId,
...
describe("simulateWorkflowPath", () => {
  describe("AC2 — linear workflow, no rules", () => {
    it("visits every section in order and leaves nothing unvisited", () => {
      const result = simulateWorkflowPath(linearThreeSections());
      expect(result.visited).toEqual(["section-a", "section-b", "section-c"]);
  ...
  describe("AC3 — forward skip_to, condition met", () => {
    it("omits the skipped section from visited and lists it in notVisited", () => {
      const { sections, steps, rules } = forwardSkipWorkflow();
      ...
      expect(result.visited).toEqual(["section-c"]);
      expect(result.notVisited).toEqual(["section-a", "section-b"]);
      expect(result.truncated).toBe(false);
    });
  ...
  describe("AC4 — backward skip_to is a no-op", () => {
    it("yields the same path as no rule at all, even though the condition is met", () => {
      const { sections, steps, rules } = backwardSkipWorkflow();
      ...
      expect(withRule.visited).toEqual(["section-a", "section-b", "section-c"]);
      expect(withRule.visited).toEqual(withoutRule.visited);
    });
  });

  describe("AC5 — section hidden by visibleIf", () => {
    it("is absent from visited and present in notVisited", () => {
      const result = simulateWorkflowPath(hiddenMiddleSectionWorkflow());
      expect(result.visited).toEqual(["section-a", "section-c"]);
      expect(result.notVisited).toEqual(["section-b"]);
    });
  });

  describe("AC6 — traversedEdges matches buildWorkflowMap's edge ids", () => {
    it("matches for the linear no-rules path", () => {
      const input = linearThreeSections();
      ...
      const { edges } = buildWorkflowMap({ sections: input.sections, steps: input.steps, rules: [] });
    ...
    it("matches for the forward-skip path, using the skip edge's own id", () => {
      ...
      const { edges } = buildWorkflowMap({ sections, steps, rules });
      const mapEdgeIds = new Set(edges.map((e) => e.id));
  ...
  describe("review defect — ambiguous skip target names the rule that actually fired", () => {
    it("labels the traversed edge with the winning rule's id, not the first array match", () => {
      const { sections, steps, rules } = ambiguousSkipTargetWorkflow();
      const result = simulateWorkflowPath({
        sections,
        steps,
        rules,
        ...
        // action/targetType/targetSectionId alone (ignoring `order` and
      ...
      expect(result.visited).toEqual(["section-d"]);
      expect(result.traversedEdges).toEqual([
        "skip:rule-from-B",
        "sequential:section-d->__complete__",
      ]);
  ...
  describe("AC7 — parity with the server's navigation decision", () => {
    ...
    it("agrees with a server-style walk over the hidden-section fixture", () => {
      const input = hiddenMiddleSectionWorkflow();
      expect(simulateWorkflowPath(input).visited).toEqual(serverStyleWalk(input));
    });
  });
  ...
  describe("AC8 — iteration cap", () => {
    it("sets truncated instead of looping forever on malformed duplicate-id input", () => {
      ...
      // Proves the walk actually ran to the cap (sections.length + 1 = 4)
      // rather than stopping early for some unrelated reason.
      expect(result.visited).toHaveLength(input.sections.length + 1);
    });
